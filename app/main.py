...
async def lifespan(app: FastAPI):
    ...
    # Pre-warm browser (optional, saves time on first request)
...
app = FastAPI(title="TDS Quiz Solver", version="0.1.0", lifespan=lifespan)
...
async def solve_quiz(request: SolveRequest, background_tasks: BackgroundTasks):
    ...
    if request.secret != settings.quiz_secret:
        logger.warning(f"⛔ Invalid secret provided: {request.secret[:3]}***")
